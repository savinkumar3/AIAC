...
def simulate_stock_data(n):
    ...
    for i in range(n):
        ...
        stocks.append({'symbol':f"STK{i+1:03d}",'open':o,'close':c,'pct_change':(c-o)/o*100})
    return stocks
...
def print_top_stocks(stocks,n=10):
    ...
    print('-'*40)
    for s in stocks[:n]: print(f"{s['symbol']:<8} {s['open']:>10.2f} {s['close']:>10.2f} {s['pct_change']:>10.2f}")

def main():
    n_stocks=10000; random.seed(123); stocks=simulate_stock_data(n_stocks)
    t=time.time(); sorted_heap=heap_sort(stocks,key=lambda x:x['pct_change']); heap_time=time.time()-t
    t=time.time(); sorted_std=sorted(stocks,key=lambda x:x['pct_change'],reverse=True); std_time=time.time()-t
    print("Top 10 Stocks by % Change (Heap Sort):"); print_top_stocks(sorted_heap,10)
    print("\nTop 10 Stocks by % Change (sorted()):"); print_top_stocks(sorted_std,10)
    print(f"\nHeap Sort Time: {heap_time:.6f} s\nsorted() Time: {std_time:.6f} s")
    t=time.time(); stock_hash=build_stock_hash(stocks); hash_build_time=time.time()-t
    test_symbol=stocks[n_stocks//2]['symbol']
    t=time.time(); found=search_stock(stock_hash,test_symbol); hash_lookup_time=time.time()-t
    t=time.time(); found_linear=next((s for s in stocks if s['symbol']==test_symbol),None); linear_search_time=time.time()-t
    ...
    while True:
        ...
        print(f"Symbol: {r['symbol']}, Open: {r['open']:.2f}, Close: {r['close']:.2f}, % Change: {r['pct_change']:.2f}" if r else "Stock symbol not found.")
